...
* 하드웨어와 응용프로그램(Applications) 사이의 소프트웨어.
  ...
  * 서로 다른 응용프로그램 사이나, 서로 다른 사용자의 사이를 참조.
...
* 하드웨어 리소스에 대한 추상화 레이어를 제공 한다.
  * 추상화를 통해 더 높은 레벨의 자원에 접근 할 수 있다.
  * 사용자 프로그램이 간단하고 이식 가능한 높은 수준의 개념을 다룰 수 있도록 한다.
  * 한정된 리소스를 무한정인것 처럼 사용할 수 있게 해준다. 

* 복잡한 리소스를 관리해준다.
  * 응용프로그램에 대한 복잡한 리소스 및 상호작용을 관리해준다.
  * 여러 응용프로그램이 함께 진행될 때, 서로 손상 없이 리소스를 공유 할 수 있게 해준다.
  * 여러 사용자가 서로에게 영향을 주지않고 리소스를 공유할 수 있게 해준다.
...
* 커널 모드와 유저 모드가 존재한다.
  ...
  * 커널 모드:수퍼바이저(Supervisor) 모드. OS에 커널이 동작된다.
    모든 하드웨어 접근 및 모든 명령어를 실행할 수 있다.
  * 유저 모드: OS제외 모든 응용프로그램 사용 가능. 
    명령어를 제한적으로 사용할 수 있다.
...
1. User program이 OS를 불러야 할 때, 혹은 User mode에서  Kernel mode로 접근 해야할 때 System call을 실행.
2. Trap instruction이 user mode 에서 kernel mode로 전환 된다.
   이 때, caller의 state(레지스터나 CPU의 상태 같은 정보)를 모두 저장시킨다.
3. kernel mode의 특성 주소지인, trap handler로 이동하게 된다.
4. parameter로 실행되어야 할 System call을 수행한다.
5. return-from-trap이 발생되어, user mode로 돌아와 system call 이후 실행 순서부터 다시 실행하기 시작한다.

* OS를 호출하는 두 가지 방법이 존재한다. :
  - Trap (System call)
    * 소프트웨어가 발생시키는 메세지로, system call이나 오류 발생 시 출력됨.
  - Interrupt (I/O)
    * 하드웨어에서 발생되는 문제.
...
* 위 계층은 CPU의 거리 순으로 볼 수도 있으며, 위로 올라갈 수록 CPU와 같아진다.
  CPU와 가까워질 수록 속도가 빠르지만 용량이 작아지고, 거리가 멀 수록 용량은 커지지만 속도가 느려진다.
* 제일 상단에 있는 Register는 CPU에 내장 되어있는 저장공간이며, 내부에 있기 때문에 가장 빠른 속도로 접근 할 수 있다.
* Cache는 프로세서가 접근하여 메모리가 존재하는지 확인 할 수 있다.
* Main Memory 부터는 CPU외부에 존재하며, 접근 속도가 Cache나 Register보다 느리다.
  하드디스크 내에 존재하는 내용의 경우, 프로그램 실행을 위해 Main Memory로 이동 된다.
...
* Virtual memory(가상 메모리)
  ...
  * 메모리에서 프로그램 실행 명령어를 읽어 오게 될 경우, 실행에 필요한 데이터를 메모리에서 읽어온 후 그 결과를 메모리에 기록하는 형식이다.
  * 운영 체제 내에서 디스크 공간을 메모리처럼 활용할 수 있는 기능을 제공하여 준다.
  * 다만 가상 메모리의 경우, 주소가 다르기 때문에 실제 메모리에 저장 될 때 매핑의 과정을 거쳐야한다.
...
* 프로세스는 실행중인 프로그램의 인스턴스이다.(실행중인 프로그램의 한 단위.)
  ...
  – 동일한 프로그램(ex: 편집기)의 복사본을 실행하는 프로세스가 있을 수 있으나, 프로세스는 대체적으로 독립적이다.

* 프로세스는 OS의 실행을 위한 추상화이다.
...
* 프로세스는..
  * 대체적으로 보호되고 가상으로 메모리에 매핑된 주소 공간을 가짐.
  * 실행중인 프로그램의 코드
  * 실행중인 프로그램의 데이터
...
* 프로그램이 실행되면, 프로세스 주소공간이 Memory에 할당 된다. 할당된 프로세스를 실행시키는 것이 CPU.
* 모든 프로세스는 자신만의 주소공간을 가지고 있으며, 프로세스 주소공간의 안은 Stack,data(heap),code로 이루어진다.
...
* 선점형 스케줄러의 경우, 실행상태의 프로세스를 준비상태로 강제  전환 할 수 있다. 비 선점 스케줄러가 대기한다.
...
* OS가 커널에서 프로세스를 표현하는 방법:
  * 각각 고유한 프로세스가 있다.
  * 각각을 나타내는 OS data structure를 프로세스 제어블록(PCB: Process Control Block)또는 Process table이라고 한다.
